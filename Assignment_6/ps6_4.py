...
def deal_hand(n):
    ...
    return hand
#TODO: mimick drawing from a pool of letters like in scrabble (e.g. can't draw 3 z's)
    ...
    * The final score is displayed.
    ...
    while len(hand) > 0 and word != '.':
        ...
        while not validWord:
            ...
            word = pick_best_word(hand,points_dict,wordsSorted)
            endTime = time.time()
...
def get_word_rearangements(word_list):
    """
    Takes each word in word_list and returns a dictionary entry.  The key is a 
    string of the letters in the word sorted and the value is the word itself.
    ...
    words_sorted = {}
    for word in word_list:
        sortedWord = string.join(sorted(word),'')
        words_sorted[sortedWord] = word
    return words_sorted

def letter_combinations(letters):
    """
    Returns list of all combinations of letters in an input string.
    ex. letter_combinations('abc') = ['a', 'ab', 'abc', 'ac', 'b', 'bc', 'c']
    
    Maintains order of input string, i.e. will maintain alphabetical order if
    input string is also ordered.
    ...
    if len(letters) == 1:
        ...
        priorCombos = letter_combinations(letters[1:])
        addedCombos = [letters[0]]
        for combo in priorCombos:
            addedCombos += [letters[0] + combo]
        return addedCombos + priorCombos
        
    
def pick_best_word(hand,points_dict,wordsSorted):
    """
    ...
    currentBest = 0
    currentWord = '.'
    sortedHand = string.join(sorted(hand),'')
    handCombos = letter_combinations(sortedHand)
    for testCombo in handCombos:
        matchWord = wordsSorted.get(testCombo,False)
        if matchWord != False:
            matchPoints = points_dict[matchWord]
            if matchPoints > currentBest:
                currentBest = matchPoints
                currentWord = matchWord
    return currentWord
            
    
    """
    combinations of string.
    combination of 'abcd' = combinations of b + combinations of a added to all b's plus single a
...
if __name__ == '__main__':
    ...
    wordsSorted = get_word_rearangements(word_list)
    play_game(word_list)

hand = deal_hand(7)
display_hand(hand)
startTime = time.time()
print pick_best_word(hand, points_dict,wordsSorted)
endTime = time.time()
print get_word_score(pick_best_word(hand, points_dict,wordsSorted),7)
