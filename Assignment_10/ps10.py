...
class Hand(object):
    ...
    def update(self, word):
        ...
        """     
        handDictCopy = copy.copy(self.handDict)
        for letter in word:
            handDictCopy[letter] = handDictCopy[letter] - 1
            if handDictCopy[letter] == 0:
                del(handDictCopy[letter])
        self.handDict = handDictCopy  #leaves original hand unchanged if method throws keyerror
    ...
    def containsLetters(self, letters):
        ...
        for letter in letters:
            if letter in handDictCopy:
                handDictCopy[letter] = handDictCopy[letter] - 1
                if handDictCopy[letter] == 0:
                    del(handDictCopy[letter])
            else: lettersThere = False
        return lettersThere                     
    ...
    def __eq__(self, other):
        ...
        """
        selfKeys = self.handDict.keys()
        otherKeys = other.handDict.keys()
        selfKeys.sort()
        otherKeys.sort()
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        if self.points < other.points: return -1
        if self.points == other.points: return 0
    def __str__(self):
        """
